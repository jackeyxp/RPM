...
{ "ER_BINLOG_UNSAFE_STATEMENT", 1592, "Unsafe statement binlogged in statement format since BINLOG_FORMAT = STATEMENT. Reason for unsafeness: %s" },
...
{ "ER_SLAVE_HEARTBEAT_VALUE_OUT_OF_RANGE", 1624, "The requested value for the heartbeat period %s %s" },
...
{ "ER_BINLOG_ROW_ENGINE_AND_STMT_ENGINE", 1661, "Cannot execute statement: binlogging impossible since both row-incapable engines and statement-incapable engines are involved." },
{ "ER_BINLOG_ROW_MODE_AND_STMT_ENGINE", 1662, "Cannot execute statement: binlogging impossible since BINLOG_FORMAT = ROW and at least one table uses a storage engine limited to statement-logging." },
{ "ER_BINLOG_UNSAFE_AND_STMT_ENGINE", 1663, "Cannot execute statement: binlogging of unsafe statement is impossible when storage engine is limited to statement-logging and BINLOG_FORMAT = MIXED.  Reason for unsafeness: %s" },
{ "ER_BINLOG_ROW_INJECTION_AND_STMT_ENGINE", 1664, "Cannot execute row injection: binlogging impossible since at least one table uses a storage engine limited to statement-logging." },
{ "ER_BINLOG_STMT_MODE_AND_ROW_ENGINE", 1665, "Cannot execute statement: binlogging impossible since BINLOG_FORMAT = STATEMENT and at least one table uses a storage engine limited to row-logging.%s" },
{ "ER_BINLOG_ROW_INJECTION_AND_STMT_MODE", 1666, "Cannot execute row injection: binlogging impossible since BINLOG_FORMAT = STATEMENT." },
{ "ER_BINLOG_MULTIPLE_ENGINES_AND_SELF_LOGGING_ENGINE", 1667, "Cannot execute statement: binlogging impossible since more than one engine is involved and at least one engine is self-logging." },
{ "ER_BINLOG_UNSAFE_LIMIT", 1668, "Statement uses a LIMIT clause. This is unsafe because the set of rows included cannot be predicted." },
{ "ER_BINLOG_UNSAFE_INSERT_DELAYED", 1669, "Statement uses INSERT DELAYED. This is unsafe because the time when rows are inserted cannot be predicted." },
{ "ER_BINLOG_UNSAFE_SYSTEM_TABLE", 1670, "Statement uses the general_log, slow_log or performance_schema table(s). This is unsafe because system tables may differ on slave." },
{ "ER_BINLOG_UNSAFE_AUTOINC_COLUMNS", 1671, "Statement invokes a trigger or a stored function that inserts into AUTO_INCREMENT column which is unsafe to binlog in STATEMENT format because slave may execute it non-deterministically." },
{ "ER_BINLOG_UNSAFE_UDF", 1672, "Statement uses a UDF. It cannot be determined if the UDF will return the same value on slave." },
{ "ER_BINLOG_UNSAFE_SYSTEM_VARIABLE", 1673, "Statement uses a system variable whose value may differ on slave." },
{ "ER_BINLOG_UNSAFE_SYSTEM_FUNCTION", 1674, "Statement uses a system function whose value may differ on slave." },
{ "ER_BINLOG_UNSAFE_NONTRANS_AFTER_TRANS", 1675, "Non-transactional reads or writes are unsafe if they occur after transactional reads or writes inside a transaction." },

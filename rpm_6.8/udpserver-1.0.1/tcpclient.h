...
public:
  ...
  void      SetScreenID(int nScreenID) { m_nScreenID = nScreenID; }
private:
...
private:
  int       doCmdScreenLogin();
  int       doCmdScreenOnLine();
  int       doCmdScreenFinish();
  int       doCmdPHPGetRoomFlow();
  ...
  int       doCmdScreenPacket(const char * lpDataPtr, int nDataSize);
  int       doTransferScreenPackToTeacher(const char * lpDataPtr, int nDataSize);
